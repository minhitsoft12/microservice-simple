  ...
  async verifyGoogleToken(idToken: string): Promise<any> {
    try {
      ...
      if (!validAudiences.includes(payload.aud)) {
        this.logger.warn(`Invalid token audience: ${payload.aud}`);
        throw new UnauthorizedException('Invalid token audience');
      }

      // Check if email is verified
      if (!payload.email_verified) {
        this.logger.warn(`Unverified email: ${payload.email}`);
      ...
      if (!allowedDomains.includes(emailDomain)) {
        this.logger.warn(
          `Unauthorized domain: ${emailDomain}, email: ${payload.email}`,
        );
      ...
      this.logger.error(
        `Google token verification error: ${error.message}`,
        error.stack,
      );
