...
export class AuthController {
  ...
  @Public()
  @Post('mobile/google')
  @ApiOperation({ summary: 'Authenticate with Google from mobile app' })
  @ApiResponse({ status: 200, description: 'Authentication successful' })
  @ApiResponse({ status: 401, description: 'Invalid Google token' })
  async mobileGoogleAuth(@Body() mobileAuthDto: MobileAuthDto) {
    this.logger.log(
      `Mobile Google auth attempt with token: ${mobileAuthDto.idToken.substring(0, 10)}...`,
    );

    try {
      const result = await this.authService.verifyGoogleToken(
        mobileAuthDto.idToken,
      );

      this.logger.log(`Verified Google token for user: ${result.email}`);

      // Login with the verified Google user data
      return this.authService.googleLogin({
        sub: result.sub,
        email: result.email,
        name: result.name,
        picture: result.picture,
        service: mobileAuthDto.platform || 'mobile',
      });
    } catch (error) {
      this.logger.error(
        `Mobile Google auth error: ${error.message}`,
        error.stack,
      );
